...
const TenantDashboard = () => {
  ...
  useEffect(() => {
    // Run both in parallel; `fetchUnits` will turn off loading
    fetchProperties();
    fetchUnits();
  }, []);
  ...
  const getUnitsForProperty = (propertyId) => {
    ...
    const filtered = units.filter((unit) => {
      const unitPropId = normalizeId(unit.propertyId);
      return unitPropId === normalizedPropertyId && unit.status === 'AVAILABLE';
    });
      ...
      <div className="premium-container">
        ...
        >
          ...
          >
            ...
            >
              {(() => {
                const availableUnitsWithRent = availableUnits.filter(
                  (unit) => unit.rentAmount && unit.rentAmount > 0
                );
                if (availableUnitsWithRent.length > 0) {
                  const minRent = Math.min(
                    ...availableUnitsWithRent.map((unit) => unit.rentAmount)
                  );
                  return `$${minRent}`;
                }
                return '$0';
                  ...
                  >
                    ...
                    >
                      ...
                      {propertyUnits.length > 0 && (
                        <div
                          className="property-badge available"
                          style={{
                            position: 'absolute',
                            top: '3.5rem',
                            left: '1rem',
                        ...
                        >
                          {propertyUnits.length} Unit
                          {propertyUnits.length > 1 ? 's' : ''} Available
                        </div>
                      )}
                    </div>
                    ...
                    <div className="property-content" style={{ padding: '2rem' }}>
                      ...
                      >
                        {property.description ||
                          'Beautiful property in a great location with modern amenities and convenient access to local facilities.'}
                      </div>
                      ...
                      {propertyUnits.length > 0 && (
                        <>
                          ...
                          <div className="property-footer">
                            <Link
                              to={`/property/${normalizeId(property._id)}`}
                              className="btn-premium btn-primary"
                              style={{
                                display: 'block',
                                width: '100%',
            ...
            >
              {availableUnits.map((unit) => {
                const property = properties.find(
                  (p) => normalizeId(p._id) === normalizeId(unit.propertyId)
                );

                return (
                  <div
                    key={normalizeId(unit._id)}
                    className="unit-card"
                    style={{
                      background: 'var(--bg-card)',
                      borderRadius: '20px',
                  ...
                  >
                    ...
                    <div className="unit-content" style={{ padding: '2rem' }}>
                      ...
                      >
                        ...
                        {unit.depositAmount || unit.rentAmount || 'N/A'}
                      </div>

                      <div className="unit-actions">
                        <Link
                          to={`/unit/${normalizeId(unit._id)}`}
                          className="btn-premium btn-primary"
                          style={{
                            display: 'block',
                            width: '100%',
